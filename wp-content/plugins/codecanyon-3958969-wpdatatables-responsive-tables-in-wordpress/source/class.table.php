...
class PHPDataTable {
	...
	/**
	 * Default sort direction for datatables
	 * @var string
	 */
	 ...
	 */
        ...
        /**
         * Name of ID column for frontend editing
         */
        ...
        */
        ...
        /**
         * Flag which defines if script blocks should be rendered
         */
         private $scriptsEnabled = true;
         
        /**
         * Flag which defines if the table should be responsive
         */
         private $_responsive = false;         
         
         /**
          * Flag which defines if the table contains no data
          */
     ...
     */
      ...
      */
      ...
      /**
       * Returns if the table is responsive
       */
     ...
     */
     public function setDefaultSortDirection($direction){
     	if(!in_array($direction, array('ASC','DESC'))){ return false; }
     ...
     */
     ...
     /**
      * Disables editing for the table
      */
    ...
    /**
     * Sets the columns possible values from an array
     * 
     * @param type $array
     * @throws Exception 
     */
    public function setColumnsPossibleValues( $array ) {
		if( empty($this->_columns) ) {
			throw new Exception('No columns in the table!');
		}
		if( !is_array($array) ) {
		...
		if( $this->_isAssoc($array) ) {
			...
			foreach( $array as $key=>$value ) {
				if(!isset($this->_columnsByKeys[$key])) { continue; }
				$this->_columnsByKeys[$key]->setPossibleValues($value);
			}
		} else{
			// if width is provided in indexed array
			foreach( $array as $key=>$value ) {
				$this->_columns[$key]->setPossibleValues($value);
			}
		}
    }    
    ...
    /**
     * Returns set column key indes
     */
     public function getColumnKeyIndex($key){
    ...
    /**
     * Method which disables the scripts render
     */
     ...
     /**
      * Method which returns if scripts are enabled
      */
	...
	public function buildByArray( $array, $tableParams = array() ) {
		if(empty($array)){
			$array = array(0 => array('No data' => 'No data'));
			$this->setNoData(true);
		}
    ...
    public function buildByQuery($query, $queryParams = array(), $tableParams = array ()) {
						...
						&& ($_GET['sSearch_'.$i] != '~') )
					{
						...
						switch($tableParams['filter_types'][$aColumns[$i]]) {
							...
							case 'date-range':
								...
								break;
							case 'select':
								$search .= $aColumns[$i]." = '".mysql_real_escape_string($_GET['sSearch_'.$i])."' ";
								break;
							case 'checkbox':
								...
								foreach($checkboxSearches as $checkboxSearch){
									...
									$search .= $aColumns[$i]." = '".mysql_real_escape_string($checkboxSearch)."' ";
									$j++;
								}
								break;
							case 'text':
        ...
        if(isset($_GET['action']) && ($_GET['action'] == 'get_wdtable')){
			...
			foreach($tableParams['data_types'] as $column_key=>$column_type){
				$col_objs[$column_key] = Column::factory( $column_type, array('header' => $tableParams['column_names'][$column_key]) );
				$col_objs[$column_key]->setInputType($tableParams['input_types'][$column_key]);
			}
    ...
    private function _getJsDataTablesView() {
        ...
        if(true || $this->_isFirst && $this->scriptsEnabled) {
	        ...
	        if($this->columnsFixed() || $this->headersFixed()){
		       wp_register_script('wpdatatables-fixedheader',PDT_JS_PATH.'jquery-datatables/FixedHeader.js',array('jquery','datatables'));
	           wp_enqueue_script('wpdatatables-fixedheader');
	        }
			if($this->filterEnabled()){
				wp_register_script('wpdatatables-advancedfilter',PDT_JS_PATH.'jquery-datatables/jquery.dataTables.columnFilter.min.js');
			...
			if($this->isEditable()){
		        ...
		        wp_register_script('jquery-fileupload',PDT_JS_PATH.'fileupload/jquery.fileupload.js',array('jquery'));
		        wp_register_script('jquery-maskmoney',PDT_JS_PATH.'maskmoney/jquery.maskMoney.js',array('jquery'));
		        wp_enqueue_script('jquery-fileupload-iframe');
		        wp_enqueue_script('jquery-fileupload');
		        wp_enqueue_script('jquery-maskmoney');
		        wp_register_style('jquery-fileupload',PDT_CSS_PATH.'jquery.fileupload.css');
    ...
    public function renderTable() {
        ...
        if($this->_isFirst) {
            ...
            $tpl->addCss(PDT_ASSETS_PATH.'css/wpdatatables.min.css');
        }
        if ($this->_tableType == self::PLAIN_TABLE) {
     ...
     */
     public function getJsonDescription(){
     	...
     	if($this->isEditable()){
     		...
     		$obj->idColumnIndex = $this->getColumnKeyIndex($this->getIdColumnKey());
     		$obj->idColumnKey = $this->getIdColumnKey();
     	}
   		$obj->spinnerSrc = PDT_ASSETS_PATH.'/img/spinner.gif';
     	...
     	$obj->dataTableParams->sDom = 'T<"clear">lfrtip';
     	...
     	if(!$this->getNoData()){
	     	$obj->dataTableParams->aoColumnDefs = json_decode('['.$this->getColumnDefs().']');
     	}
     	$obj->dataTableParams->bAutoWidth = false;
     	if($this->sortingEnabled() && !is_null($this->getDefaultSortColumn())){
     	...
     	if($this->tableToolsEnabled()){
     		$obj->dataTableParams->oTableTools = array(
     			'sSwfPath' => PDT_JS_PATH.'jquery-datatables/media/swf/copy_cvs_xls_pdf.swf',
     			'aButtons' => array(
     	...
     	if($this->serverSide()){
     		$obj->serverSide = true;
     		$obj->dataTableParams->bProcessing = true;
     	...
     	if($this->columnsFixed()){
     		$obj->columnsFixed = (int)$this->columnsFixed() ;
     		$obj->offsetLeft = $this->getLeftOffset();
     		$obj->offsetTop = $this->getTopOffset();
     	}else{
     		$obj->columnsFixed = 0;
     	}
