...
<select name="field_options[hide_field_<?php echo esc_attr( $field['id'] ) ?>][]" onchange="frmGetFieldValues(this.value,<?php echo (int) $field['id'] ?>,<?php echo esc_attr( $meta_name ) ?>,'<?php echo esc_attr( $field['type'] ) ?>')">
	...
	foreach ( $form_fields as $ff ) {
		if ( $ff->id == $field['id'] || FrmField::is_no_save_field( $ff->type ) || in_array( $ff->type, array( 'file', 'rte', 'date' ) ) || ( $ff->type == 'data' && ( ! isset( $ff->field_options['data_type'] ) || $ff->field_options['data_type'] == 'data' || $ff->field_options['data_type'] == '' ) ) ) {
            continue;
        }
