...
function education_load_scripts() {
    ...
    wp_enqueue_script( 'datatables', 'https://cdn.datatables.net/t/ju/jszip-2.5.0,pdfmake-0.1.18,dt-1.10.11,af-2.1.1,b-1.1.2,b-colvis-1.1.2,b-flash-1.1.2,b-html5-1.1.2,b-print-1.1.2,cr-1.3.1,fc-3.2.1,fh-3.1.1,kt-2.1.1,r-2.0.2,rr-1.1.1,sc-1.4.1,se-1.1.2/datatables.min.js', array( 'jquery' ), '1.10.15' );
    wp_enqueue_script( 'jquery-matchHeight', get_bloginfo( 'stylesheet_directory' ) . '/js/jquery.matchHeight.js', array( 'jquery' ), '0.6.0' );
...
// add_action('frm_after_create_entry', 'uag_user_register', 40, 2);
function uag_user_register ( $entry_id, $form_id ) {
	$user = get_user_by( 'login', $_POST['item_meta'][4889] );
	$user_id = $user->ID;
	echo "user_id: $user_id";
    if ($user_id != null) {
    	$key = 'security_group';
  		$single = true;
    	$security_group = $_POST['item_meta'][4903];
...
function frmreg_new_role($role, $atts) {
	...
	if ( $atts['form']->id == 122 && isset( $atts['entry'] ) ) {
		$user_login = FrmProEntriesController::get_field_value_shortcode( array( 'field_id' => 4889, 'entry_id' => $atts['entry']->id, 'show' => 'value' ) );
		$user = get_user_by( 'login', $user_login );
		$user_id = $user->ID;
		$user_role = FrmProEntriesController::get_field_value_shortcode( array( 'field_id' => 4905, 'entry_id' => $atts['entry']->id, 'show' => 'value' ) );
		if ($user_id != null) {
			$key = 'security_group';
			$single = true;
			$security_group = get_user_meta( $user_id, $key, $single );
			if ( $group = Groups_Group::read_by_name( $security_group ) ) {
				$result = Groups_User_Group::create( array( "user_id"=>$user_id, "group_id"=>$group->group_id ) );
			}
		}
