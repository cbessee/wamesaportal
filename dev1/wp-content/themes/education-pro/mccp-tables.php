...
function create_mccp_enrollment($entry_id, $form_id){
  if($form_id == 74){ //change 4 to the form id of the form to copy
    global $wpdb;
    $mccp_enrollment_db = $wpdb->prefix.'mccp_enrollment';
    ...
    $wpdb->insert($mccp_enrollment_db, $values);
  }
}
...
function update_mccp_enrollment($entry_id, $form_id){
  if($form_id == 74){ //change 4 to the form id of the form to copy
    ...
    $sql = $wpdb->prepare("SELECT count(*) FROM %s WHERE entry_id = %s AND post_type != 'post'", $mccp_enrollment_db, $entry_id);
...
function create_mccp_facultyadvisor($entry_id, $form_id){
  if($form_id == 76){ //change 4 to the form id of the form to copy
    global $wpdb;
    $mccp_facultyadvisor_db = $wpdb->prefix.'mccp_facultyadvisor';
...
function update_mccp_facultyadvisor($entry_id, $form_id){
  if($form_id == 76){ //change 4 to the form id of the form to copy
    ...
    $mccp_facultyadvisor_db = $wpdb->prefix.'mccp_facultyadvisor';
    //get the number of revisions already in the database
    $sql = $wpdb->prepare("SELECT count(*) FROM %s WHERE entry_id = %s AND post_type != 'post'", $mccp_facultyadvisor_db, $entry_id);
    $rec_cnt = $wpdb->get_var($sql);
    $num_revisions = (int) $rec_cnt + 1;
    $post_type = $entry_id . "-revision-v" . (string) $num_revisions;
    ...
    $wpdb->insert($mccp_facultyadvisor_db, $values);
  }
}
...
function create_mccp_exit($entry_id, $form_id){
  if($form_id == 75){ //change 4 to the form id of the form to copy
    global $wpdb;
    $mccp_enrollment_db = $wpdb->prefix.'mccp_enrollment';
    $mccp_exit_db = $wpdb->prefix.'mccp_exit';
    ...
    $wpdb->insert($mccp_exit_db, $values);
    ...
    $sql = $wpdb->prepare("SELECT max(created_at) FROM %s WHERE entry_id = %s", $mccp_enrollment_db, $_POST['item_meta'][3838]);
    ...
    $wpdb->update($mccp_enrollment_db, $data, $where);
  }
}
...
function update_mccp_exit($entry_id, $form_id){
  if($form_id == 75){ //change 4 to the form id of the form to copy
    ...
    $mccp_exit_db = $wpdb->prefix.'mccp_exit';
    //get the number of revisions already in the database
    $sql = $wpdb->prepare("SELECT count(*) FROM %s WHERE entry_id = %s AND post_type != 'post'", $mccp_exit_db, $entry_id);
    $rec_cnt = $wpdb->get_var($sql);
    $num_revisions = (int) $rec_cnt + 1;
    $post_type = $entry_id . "-revision-v" . (string) $num_revisions;
...
function create_mccp_activities($entry_id, $form_id){
  if($form_id == 73){ //change 4 to the form id of the form to copy
    global $wpdb;
    $mccp_enrollment_db = $wpdb->prefix.'mccp_enrollment';
    $mccp_activities_db = $wpdb->prefix.'mccp_activities';
    ...
    $wpdb->insert($mccp_activities_db, $values);
    ...
    // first get the latest created_at date
    $sql = $wpdb->prepare("SELECT max(created_at) FROM %s WHERE entry_id = %s", $mccp_enrollment_db, $_POST['item_meta'][3645]);
    $max_dt = $wpdb->get_var($sql);
    // now that we have the latest enrollment entry, we can update the table
...
function update_mccp_activities($entry_id, $form_id){
  if($form_id == 73){ //change 4 to the form id of the form to copy
    ...
    $mccp_activities_db = $wpdb->prefix.'mccp_activities';
    //get the number of revisions already in the database
    $sql = $wpdb->prepare("SELECT count(*) FROM %s WHERE entry_id = %s AND post_type != 'post'", $mccp_activities_db, $entry_id);
    $rec_cnt = $wpdb->get_var($sql);
    $num_revisions = (int) $rec_cnt + 1;
    $post_type = $entry_id . "-revision-v" . (string) $num_revisions;
...
function create_mccp_persistence($entry_id, $form_id){
  if($form_id == 77){ //change 4 to the form id of the form to copy
    global $wpdb;
    $mccp_enrollment_db = $wpdb->prefix.'mccp_enrollment';
    $mccp_persistence_db = $wpdb->prefix.'mccp_persistence';
    ...
    $wpdb->insert($mccp_persistence_db, $values);
    ...
    // first get the latest created_at date
    $sql = $wpdb->prepare("SELECT max(created_at) FROM $s WHERE entry_id = %s", $mccp_enrollment_db, $_POST['item_meta'][3978]);
    $max_dt = $wpdb->get_var($sql);
    // now that we have the latest enrollment entry, we can update the table
...
function update_mccp_persistence($entry_id, $form_id){
  if($form_id == 77){ //change 4 to the form id of the form to copy
    ...
    $mccp_persistence_db = $wpdb->prefix.'mccp_persistence';
    //get the number of revisions already in the database
    $sql = $wpdb->prepare("SELECT count(*) FROM %s WHERE entry_id = %s AND post_type != 'post'", $mccp_persistence_db, $entry_id);
    $rec_cnt = $wpdb->get_var($sql);
    $num_revisions = (int) $rec_cnt + 1;
    $post_type = $entry_id . "-revision-v" . (string) $num_revisions;
