...
function create_k12_enrollment($entry_id, $form_id){
  if($form_id == 69){
    ...
    $wpdb->insert('wp_k12_enrollment', $values);
  }
}
...
function update_k12_enrollment($entry_id, $form_id){
  if($form_id == 69){ //change 4 to the form id of the form to copy
    ...
    $sql = $wpdb->prepare("SELECT count(*) FROM wp_k12_enrollment WHERE entry_id = %s AND post_type != 'post'", $entry_id);
...
function create_k12_teacherprofile($entry_id, $form_id){
  if($form_id == 72){ //change 4 to the form id of the form to copy
    ...
    $wpdb->insert('wp_k12_teacherprofile', $values);
  }
}
...
function update_k12_teacherprofile($entry_id, $form_id){
  if($form_id == 72){ //change 4 to the form id of the form to copy
    global $wpdb;
    //get the number of revisions already in the database
    $sql = $wpdb->prepare("SELECT count(*) FROM wp_k12_teacherprofile WHERE entry_id = %s AND post_type != 'post'", $entry_id);
    $rec_cnt = $wpdb->get_var($sql);
    $num_revisions = (int) $rec_cnt + 1;
    $post_type = $entry_id . "-revision-v" . (string) $num_revisions;
...
function create_k12_exit($entry_id, $form_id){
  if($form_id == 71){ //change 4 to the form id of the form to copy
    ...
    $wpdb->insert('wp_k12_exit', $values);
    ...
    $sql = $wpdb->prepare("SELECT max(created_at) FROM wp_k12_enrollment WHERE entry_id = %s", $_POST['item_meta'][3505]);
    ...
    $wpdb->update('wp_k12_enrollment', $data, $where);
  }
}
...
function update_k12_exit($entry_id, $form_id){
  if($form_id == 71){ //change 4 to the form id of the form to copy
    global $wpdb;
    //get the number of revisions already in the database
    $sql = $wpdb->prepare("SELECT count(*) FROM wp_k12_exit WHERE entry_id = %s AND post_type != 'post'", $entry_id);
    $rec_cnt = $wpdb->get_var($sql);
    $num_revisions = (int) $rec_cnt + 1;
    $post_type = $entry_id . "-revision-v" . (string) $num_revisions;
...
function create_k12_activities($entry_id, $form_id){
  if($form_id == 70){ //change 4 to the form id of the form to copy
    ...
    $wpdb->insert('wp_k12_activities', $values);
    ...
    // first get the latest created_at date
    $sql = $wpdb->prepare("SELECT max(created_at) FROM wp_k12_enrollment WHERE entry_id = %s", $_POST['item_meta'][3499]);
    $max_dt = $wpdb->get_var($sql);
    // now that we have the latest enrollment entry, we can update the table
...
function update_k12_activities($entry_id, $form_id){
  if($form_id == 70){ //change 4 to the form id of the form to copy
    global $wpdb;
    //get the number of revisions already in the database
    $sql = $wpdb->prepare("SELECT count(*) FROM wp_k12_activities WHERE entry_id = %s AND post_type != 'post'", $entry_id);
    $rec_cnt = $wpdb->get_var($sql);
    $num_revisions = (int) $rec_cnt + 1;
    $post_type = $entry_id . "-revision-v" . (string) $num_revisions;
...
function create_k12_persistence($entry_id, $form_id){
  if($form_id == 68){ //change 4 to the form id of the form to copy
    ...
    $wpdb->insert('wp_k12_persistence', $values);
    ...
    // first get the latest created_at date
    $sql = $wpdb->prepare("SELECT max(created_at) FROM wp_k12_enrollment WHERE entry_id = %s", $_POST['item_meta'][3399]);
    $max_dt = $wpdb->get_var($sql);
    // now that we have the latest enrollment entry, we can update the table
...
function update_k12_persistence($entry_id, $form_id){
  if($form_id == 68){ //change 4 to the form id of the form to copy
    global $wpdb;
    //get the number of revisions already in the database
    $sql = $wpdb->prepare("SELECT count(*) FROM wp_k12_persistence WHERE entry_id = %s AND post_type != 'post'", $entry_id);
    $rec_cnt = $wpdb->get_var($sql);
