...
DEFAULT_HTML;
    ...
    */
    ...
    public static function replace_html_shortcodes($html, $field, $atts) {
        if ( 'divider' == $field['type'] ) {
			...
			// if the HTML ends with a div, remove it (end_divider will add close div)
			self::remove_close_div( $html );
		...
		} else if ( FrmField::is_option_true( $field, 'conf_field' ) ) {//Add confirmation field
	...
	*/
	private static function remove_close_div( &$html ) {
	...
	*/
	public static function get_data_value( $value, $field, $atts = array() ) {
        ...
        if ( is_numeric( $value ) && ( ! isset( $field->field_options['form_select'] ) || $field->field_options['form_select'] != 'taxonomy' ) && $linked_field_id ) {
	...
	*/
    ...
    public static function get_field_options( $form_id, $value = '', $include = 'not', $types = array() ) {
		...
		if ( empty( $types) ) {
			$types = array( 'break', 'divider', 'end_divider', 'data', 'file', 'captcha' );
