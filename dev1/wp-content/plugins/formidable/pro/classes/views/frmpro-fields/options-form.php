...
<?php } else if ( $field['type'] == 'file' ) { ?>
    ...
    <tr><td><label><?php _e( 'Email Attachment', 'formidable' ) ?></label></td>
        <td><label for="attach_<?php echo esc_attr( $field['id'] ) ?>"><input type="checkbox" id="attach_<?php echo esc_attr( $field['id'] ) ?>" name="field_options[attach_<?php esc_attr( $field['id'] ) ?>]" value="1" <?php echo ( isset( $field['attach'] ) && $field['attach'] ) ? 'checked="checked"' : ''; ?> /> <?php _e( 'attach this file to the email notification', 'formidable' ) ?></label></td>
    </tr>
    <?php if ( $mimes ) { ?>
